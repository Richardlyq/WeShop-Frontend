...
const showAssignRole = async row => {
  ...
  userRoleIds.value = data.sysRoleIds
}
...
//因为上传头像不是通过axios调用的,axios没法直接上传，所以需要用普通方式手动上传，
//用普通方式不会传请求头信息（token信息），所以以下是手动传token到请求头中的方式
const headers = {
...
//点击修改按钮，数据回显
const editShow = (row)=>{
    sysUser.value = {...row} //...是对象拓展运算符，作用是将row里面的值复制
...
//提交方法，发送axios请求实现添加
const submit = async () =>{
    if (!sysUser.value.id){ //如果sysUser中没有id，则是添加操作
